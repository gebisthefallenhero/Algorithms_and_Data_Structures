...
public class MinMaxFlow{
    ...
    public void printMaxFlow(){
        ...
        findMaxFlow();
        System.out.println("\n" + flowGraph.graphName + " Max Flow SPACE " + flowGraph.maxFlowIntoSink + " ASSIGNED " + flowIntoSink);
        updateCost();
    ...
    private void updateCost(){
        for (GraphNode node: flowGraph.G){
            for (EdgeInfo edge: node.succ){
                int flowAssigned = residual[edge.from][edge.to];
                if (flowAssigned <= edge.capacity && edge.capacity != 0 && flowAssigned != 0){
